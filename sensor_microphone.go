...
// microphoneSensor uses microphone input to detect "slaps" (loud sounds)
// Shared implementation for Linux and Windows via PortAudio
type microphoneSensor struct {
	threshold    int
...
// newMicrophoneSensor creates a microphone sensor with configurable threshold
func newMicrophoneSensor(threshold int) (*microphoneSensor, error) {
...
func (s *microphoneSensor) Read() (eventDetected bool, severity string, amplitude float64, err error) {
...
func (s *microphoneSensor) calculatePeakAmplitude() int {
	maxAmp := 0
...
func (s *microphoneSensor) Close() error {
	if s.stream != nil {
		s.stream.Stop()
		s.stream.Close()
	}
...
func checkPlatformRequirements() error {
	...
	// Don't terminate here - let newMicrophoneSensor handle it
	portaudio.Terminate()
	return nil
}
...
func (s *microphoneSensor) setThreshold(threshold int) {
